...
function PricePill({ label, value, unit = '/quintal' }: { label: string; value: number; unit?: string }) {
  return (
    <div className="flex flex-col items-center rounded-lg border bg-field-50 border-field-200 text-field-900 px-3 py-2">
...
function NoPricesTodayModal({ onDismiss, previousDayDate, cropName }: { onDismiss: () => void; previousDayDate?: string; cropName?: string }) {
  return (
      ...
      <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-white shadow-2xl">
        ...
        <div className="p-6">
          <p className="text-sm text-earth-700">
            Price data for <strong>{cropName}</strong> is not yet available for today. We're showing prices from{' '}
...
export function MandiPricesTab({ data, loading, error, crop, onRefresh }: MandiPricesTabProps) {
  ...
  // Use crop display name from API if available, otherwise use prop
  const displayCropName = data?.cropDisplayName || crop || 'Crop';
  const priceUnit = data?.priceUnit?.replace('₹/', '') || 'quintal';
      ...
      {data && data.prices.length > 0 ? (
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-3 gap-3">
                <PricePill label="Avg Modal" value={avgModal} unit={unitLabel} />
                <PricePill label="Best Price" value={maxModal} unit={unitLabel} />
                <PricePill label="Lowest" value={minModal} unit={unitLabel} />
              </div>
            );
          })()}
              ...
              <div key={i} className="rounded-xl border border-earth-200 bg-white p-4 shadow-sm">
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-earth-400">modal/{priceUnit}</p>
                  </div>
                </div>
        ...
        <div className="rounded-xl border border-earth-200 bg-earth-50 p-6 text-center">
          <p className="text-sm text-earth-600">
            No price records found for <strong>{displayCropName}</strong> in <strong>{data.detectedState}</strong> in the last few days.
          </p>
          <p className="mt-1 text-xs text-earth-500">
            This crop may not have active market listings in this region. Try refreshing, or check Agmarknet directly for regional price data.
          </p>
          <a
